...
There are two creator-operation profiles:
...
Idea generation currently uses Workflow V1 with two additional isolated profiles:

- `idea-inventor`: expands mechanically distinct gameplay kernels without producing polished proposals;
- `idea-converger`: performs red-team filtering and turns the surviving kernels into executable game specifications.

There is no independent `idea-auditor` call in V1. The backward-compatible `idea.audit` response field is a structured verdict produced inside the converger stage. New tasks checkpoint `invention` and `convergence`; completed V2 records remain readable, while retried or unfinished V2 tasks restart on V1 with incompatible checkpoints cleared.
...
The proposed PostgreSQL + Inngest migration, multi-user task API, state model and rollout plan are specified in [`BUSINESS-API-TASK-MANAGEMENT.md`](BUSINESS-API-TASK-MANAGEMENT.md).
