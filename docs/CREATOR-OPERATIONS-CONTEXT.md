...
## 最新 PRD 1.0 对齐（Agent 范围）

本节整理自《Loopit 创作者运营Agent平台 PRD 1.0》。PRD 描述的是多个系统共同组成的企业级运营平台；本仓库只实现其中的 Agent 个性化层。文档中出现的其他系统、业务名词和产品能力，作为集成背景理解，不默认在本项目内实现。
...
### Agent 运行时契约（PRD 1.0 建议形态）

Agent 不应只接收一段自由文本，而应由执行中心组装一次不可变的运行上下文。建议至少包含：

- `runId`、`agentVersion`、`campaignId`、`campaignVersion` 和 `promptTemplateVersion`；
- 已确认的活动摘要、任务说明、有效期、候选 Power/Skill/Template、固定文案和官方入口；
- 创作者 UID、语言、时区和上游已经确认的 `eligibility` / `allowedActions`；
- 用于个性化的作品或行为证据，每条证据带来源、数据时间和可选过期时间；
- 频控、静默窗口、退订、审核方式等已经由执行中心判断的结果。

Creator Score、内部标签、原始年龄和积分计算过程不应放进 Agent Memory。年龄相关规则应在活动中台完成，Agent 只接收允许的路线或动作结果，避免模型自行猜测未成年人状态。

Agent 的输出建议固定为结构化结果，而不是只返回文本：

```json
{
  "decision": "SEND | NO_OUTREACH",
  "message": "可直接发送的个性化文案",
  "reasonCode": "recent_growth",
  "evidenceRefs": ["work_consumption:..."],
  "campaignVersion": "..."
}
```

`SEND` 只表示 Agent 认为存在明确且有价值的触达理由，不代表已经发送。真正发送前仍必须由执行中心完成敏感词、安全、链接、频控、去重、审核和权限校验。活动 ID、任务、积分、进度、按钮和跳转地址由活动中台或客户端使用权威配置组装，不能由模型自由生成。

### PRD 1.0 待确认问题

以下问题会直接影响 Agent 的输入输出契约，建议在开发活动中台和 Agent 联调前确认：

#### 必须先确认

1. **Agent 的决策权限**：Agent 是否可以返回 `SEND / NO_OUTREACH`，还是只负责生成文案，发送决策完全由执行中心确定？两者的审计和失败处理不同。
2. **资格上下文**：Agent 收到的是最终 `eligible / ineligible` 结果，还是还要看到命中和排除条件？哪些条件允许展示给创作者？
3. **年龄信息边界**：Agent 是否只接收 `ageRoute / allowedActions`，还是允许接收年龄段？原始年龄是否永远禁止进入 Prompt、Memory 和 Langfuse？
4. **证据协议**：作品、行为和评论证据由哪个服务聚合？统一字段、时间窗、时区、最大条数和过期规则是什么？
5. **运营提示词模板**：运营可编辑的模板能使用哪些变量？是否需要版本、审批、预览、敏感词校验和回滚？模板是否只能作为用户上下文，不能修改 System Prompt、Skill 和工具权限？
6. **活动卡片责任**：Agent 只生成卡片配套文案，还是可以选择卡片模板和 CTA？活动 ID、任务、奖励、进度和 action URL 是否全部由中台返回并锁定？
7. **输出格式**：是否统一采用结构化 JSON？`NO_OUTREACH` 的原因码、文案最大长度、语言、Markdown、表情和链接规则是什么？
8. **版本锁定**：一次批次需要锁定哪些版本：活动、任务、积分规则、Agent Profile、System Prompt、Skill、运营模板和模型？回滚时运行中的批次如何处理？
9. **UID 试跑语义**：试跑是否完整经过资格、频控、Agent、内容校验和积分试算？哪些外部调用必须强制 dry-run，如何保证绝不发 IM 和积分？
10. **失败策略**：模型超时、证据服务失败、输出校验失败或人工审核拒绝时，统一是跳过、重试还是进入待办？重试是否允许重新生成不同文案？

#### 可以先按默认方案推进

- Agent 只读，不提供报名、任务完成、发积分、领取奖励和发送 IM 的写工具。
- Agent 只接收上游确认后的 `allowedActions`，不接收 Creator Score，也不自行推导年龄、资格和积分。
- Agent 返回 `SEND / NO_OUTREACH + 文案 + reasonCode + evidenceRefs`；`SEND` 不等于实际发送。
- 活动卡片和所有权威业务字段由活动中台组装，Agent 只能引用已发布的 `campaignVersion`。
- Agent 失败默认不发送、不发奖，进入可重试或人工审核队列。
- 运营模板作为受限数据注入用户上下文，System Prompt、Skill 骨架、工具白名单和停止条件仍由 Git/版本化 Skill 维护。
- Gemini 运行时保持稳定的 System Prompt 前缀，动态活动和 UID 证据放在本轮上下文，避免降低 Prompt Cache 命中率。
