...
例如公开作品画像、消费指标、Power、Hashtag 和评论摘要共同组成“作品分析”，应由 `creator_project_analyze` 聚合，而不应要求主 Agent 依次调用多个底层查询。
...
| --- | --- | --- |
| `creator_project_analyze` | `query_public_work` | 分析本人或他人的公开作品 |
| `creator_comments_analyze` | `analyze_work_comments` | 总结公开作品点赞 Top50 评论 |
...
Creator IM Profile 只加载前三个；平台侧 Outreach Profile 只加载 `creator_activity_status`。两个 Profile 不共享数据工具，避免把活动后台状态误暴露到一期 IM Agent。
...
- 活动状态只能由平台侧 `creator_activity_status` 读取，Creator IM 不据此替代活动后台。
